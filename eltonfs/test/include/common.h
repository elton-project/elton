...
#define PRINT_ERROR()                                                          \
  do {                                                                         \
    perror("ERROR");                                                           \
    fprintf(stderr, "  Occurred on %s (%s:%d)\n", __func__, __FILE__,          \
            __LINE__);                                                         \
  } while (0)
  ...
  ({                                                                           \
    ...
    if (ret == -1 && errno == expect)                                          \
      ...
      fprintf(stderr,                                                          \
              ...
              "  Occurred on %s (%s:%d)\n"                                     \
              "  Expr: %s\n",                                                  \
              ret, __func__, __FILE__, __LINE__, #expr);                       \
      exit(1);                                                                 \
    }                                                                          \
    ok;                                                                        \
  })
...
#define ASSERT(expr, msg)                                                      \
  if (!(expr)) {                                                               \
    fprintf(stderr,                                                            \
            ...
            "  Occurred on %s (%s:%d)\n"                                       \
            "  Expr: %s\n",                                                    \
            msg, __func__, __FILE__, __LINE__, #expr);                         \
    exit(1);                                                                   \
  }
  ...
  ({                                                                           \
    ...
    if ((expect) != ret) {                                                     \
      fprintf(stderr,                                                          \
              ...
              "  Result: %d\n"                                                 \
              "  Expected: %ld\n",                                             \
              msg, __func__, __FILE__, __LINE__, #expr, ret,                   \
              (long int)(expect));                                             \
      exit(1);                                                                 \
    }                                                                          \
  })
